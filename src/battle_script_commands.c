...
static void atk01_accuracycheck(void)
{
    ...
    else
    {
        ...
        if (atkAbility == ABILITY_HUSTLE && IS_MOVE_PHYSICAL(gCurrentMove))
...
static void atk06_typecalc(void)
{
    u8 moveType;
    GET_MOVE_TYPE(gCurrentMove, moveType);
    gMultiplierTypeEffectiveness = CalculateTypeEffectiveness(gCurrentMove, moveType, gBattlerAttacker, gBattlerTarget, TRUE);
...
static void atk3D_end(void)
{
    ...
    gMoveResultFlags = 0;
    gActiveBattler = 0;
    gCurrentActionFuncId = B_ACTION_11;
}
...
static void atkAE_healpartystatus(void)
{
    ...
    if (gCurrentMove == MOVE_HEAL_BELL)
    {
            ...
            && !(gAbsentBattlerFlags & gBitTable[gActiveBattler]))
        {
            ...
            {
                RecordAbilityBattle(gActiveBattler, ABILITY_SOUNDPROOF);
                gBattleCommunication[MULTISTRING_CHOOSER] |= 2;
            }
        }
...
static void atkB7_presentdamagecalculation(void)
{
    ...
    if (rand < 204)
    {
        gBattleStruct->presentBasePower = dmg;
